...
const Navbar = () => {
  ...
  useEffect(() => {
    try {
      const encryptedData = localStorage.getItem("tredingWebUser");
      if (encryptedData) {
        const SECRET_KEY = "treding_web_secret_key";
        const bytes = CryptoJS.AES.decrypt(encryptedData, SECRET_KEY);
        const decryptedStr = bytes.toString(CryptoJS.enc.Utf8);
        const decompressed = pako.inflate(
          new Uint8Array(decryptedStr.split("").map((c) => c.charCodeAt(0))),
          { to: "string" },
        );
  ...
  const handleLogout = () => {
    localStorage.removeItem("tredingWebUser");
    navigate("/login");
  };
